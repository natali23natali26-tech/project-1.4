...
def test_product_add_method_type_error():
    """Проверяет, что __add__ выбрасывает TypeError при сложении с не-Product."""
    product = Product("Тест", "Описание", 1000.0, 1)
...
def test_product_add_method_different_types_error():
    """Проверяет, что __add__ не позволяет складывать разные типы продуктов."""
    class OtherProduct(Product):
        pass

    a = Product("A", "Описание", 100, 1)
    b = OtherProduct("B", "Описание", 200, 1)
...
@pytest.mark.parametrize(
    ...
    [
        ("iPhone", "Флагман", 100000.0, 5, 98.5, "15 Pro", 512, "Чёрный"),
        ("Samsung", "Android", 80000.0, 3, "Snapdragon", "S23", 256, "Белый"),
    ]
)
def test_smartphone_initialization(name, description, price, quantity,
    ...
    assert phone.efficiency == str(efficiency)  # всегда строка
...
@pytest.mark.parametrize(
    ...
    [
        ("Газон", "Зелёный", 500.0, 10, "Россия", "14 дней", "Зелёный"),
        ("Трава", "Выносливая", 400.0, 20, "Германия", "10 дней", "Тёмно-зелёный"),
    ]
)
...
def test_lawngrass_repr():
    ...
    grass = LawnGrass("Газон", "Зелёный", 500.0, 10,
                      "Россия", "14 дней", "Зелёный")
    expected = ("LawnGrass(name='Газон', price=500.0, quantity=10, "
                "country='Россия', germination_period='14 дней', color='Зелёный')")
...
def test_category_add_product_type_error():
    """Проверяет, что add_product выбрасывает TypeError при передаче не-Product."""
...
def test_category_and_product_count_incremented_correctly():
    ...
    Category("Кат1", "Описание", [product1, product2])
    Category("Кат2", "Описание", [product3])
...
def test_new_product_without_list():
    """Проверяет, что new_product создаёт продукт, если список не передан."""
    data = {
        "name": "Новый телефон",
        "description": "Флагман",
        "price": 90000.0,
